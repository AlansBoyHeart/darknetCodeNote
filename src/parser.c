...
layer parse_region(list *options, size_params params)
{
    ...
    char *tree_file = option_find_str(options, "tree", 0);
    if (tree_file) l.softmax_tree = read_tree(tree_file);  //read_tree在tree.c中
    char *map_file = option_find_str(options, "map", 0);
    if (map_file) l.map = read_map(map_file);  //read_map在utils.c中
...
void parse_net_options(list *options, network *net)  ////参数为（二级链表的头指针，为net开辟的内存地址）
{
    //option_find_xxx从已经读取过的配置文件中取出网络结构的具体数值。
    net->batch = option_find_int(options, "batch",1);  //options为二级链表的头指针
...
{
    list *sections = read_cfg(filename);  //读取出来所有的配置文件，返回一个包含两级链表的大链表的头指针。
    node *n = sections->front;  //头指针指向的头结点赋值给n
    if(!n) error("Config file has no sections");
    network *net = make_network(sections->size - 1);  //为*net的各个参数，包括n,layers,seen,t,cost开辟内存
    net->gpu_index = gpu_index;
    size_params params;  //typedef struct size_params{int batch;int inputs;int h;int w;int c;int index; nt time_steps;network *net;} size_params;

    section *s = (section *)n->val;  //把头结点的值强制转换成section*结构体格式，并赋值给*s
    list *options = s->options;  //把下一级链表的头指针赋值给*options
    //如果头结点section* s的type值不是[net]或[network]，则报错。
    if(!is_network(s)) error("First section must be [net] or [network]");
    parse_net_options(options, net);  //参数为（一级链表的头指针，为net开辟的内存地址）
                                     //从已经读取过的配置文件中取出网络结构中参数的具体数值放进net中
    ...
    while(n){
        ...
        options = s->options;
        layer l = {0};
        LAYER_TYPE lt = string_to_layer_type(s->type);  //传入的是字符串[yolo]或[convolution]或[net].返回对应枚举类型的整型序号
        if(lt == CONVOLUTIONAL){
    ...
    free_list(sections);
    layer out = get_network_output_layer(net);  //network.c。如果net->layers[i].type 的值为COST，返回net->layers[i]
    net->outputs = out.outputs;  //把输出层赋值给net->outputs
    net->truths = out.outputs;   //把输出层赋值给net->truths
    if(net->layers[net->n-1].truths) net->truths = net->layers[net->n-1].truths;  //如果网络最后一层的truths值存在，
                                                                                 //则网络的最后一层。truths赋值给net->truths
...
//读取配置文件返回配置文件的链表，整个结构是：先是一个打链表串起来整个结构的大模块，包括[net],
//[convolutional],[shortcut],[yolo],每一个模块下又是一个链表，串起来所有的模块下的信息。batch=64  subdivisions=16
//width=608.      数据的格式实现方式list指向链表的头尾节点，每个节点中有一个前节点和后节点，还有一个value，value指向
//section类型的current，current包含一个字符指针char *type和一个list *options。options指向下一级的链表的头尾节点。
list *read_cfg(char *filename)
{
    ...
    char *line;
    int nu = 0;
    list *options = make_list();  //开辟一块内存
    section *current = 0;  //typedef struct{char *type;list *options;}section;

    //如果读取到了一行数据，执行循环。这里仅仅只是把一行数据以字符串的格式读取出来
    while((line=fgetl(file)) != 0){   //fgetl在utils.c中
        ++ nu;
        strip(line);
        switch(line[0]){
            case '[':
                current = malloc(sizeof(section));
                list_insert(options, current);  //头指针options指向链表的尾节点的val的值为current
                                               //list_inser把current插入到第一级链表的尾部。
                current->options = make_list();  //options指向下一级链表的头指针
                current->type = line;  //把[net]或[convolutional]或[shortcut]或[yolo]赋值给type
                break;
            case '\0':
            case '#':
            case ';':
                ...
                break;
            default:
                if(!read_option(line, current->options)){  //传入的数据是（batch=64或subdivisions=16，下一级链表的头指针）
                                                           //操作成功返回1，不成功则返回0
...
void transpose_matrix(float *a, int rows, int cols)  //l.weights, l.c*l.size*l.size, l.n
{
...
void load_convolutional_weights(layer l, FILE *fp)  //layer是网络模型的某一层，fp是指针指向二进制权重文件的地址
{
    if(l.binary){
    ...
    if (l.batch_normalize && (!l.dontloadscales)){
        ...
        if(0){
            fill_cpu(l.n, 0, l.rolling_mean, 1);  //fill_cpu函数内容（N，ALPHA，X，INCX）：for(i = 0; i < N; ++i) X[i*INCX] = ALPHA;
    ...
    if (l.flipped) {
        transpose_matrix(l.weights, l.c*l.size*l.size, l.n);  //a,rows,cols
    }
...
void load_weights_upto(network *net, char *filename, int start, int cutoff)  //参数为（网络模型net，weights文件的路径，
                                                                             //0，网络的层数）
{
#ifdef GPU
    if(net->gpu_index >= 0){
    ...
    int minor;
    int revision;
    fread(&major, sizeof(int), 1, fp);  //fread和fwrite是二进制文件io的函数，读取1次大小为四个字节的数据
