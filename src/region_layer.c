...
box get_region_box(float *x, float *biases, int n, int index, int i, int j, int w, int h, int stride)  //(l.output,l.biases,n,box_index，col,row,l.w=17,l.h=17,l.w*l.h=17*17)
{
    box b;
...
int entry_index(layer l, int batch, int location, int entry)  //参数为（最后一层，0，n*l.w*l.h + i，4）
{
    int n =   location / (l.w*l.h);  //n = n,意思是取出n*l.w*l.h + i中n的值
    int loc = location % (l.w*l.h);  //loc = i意思是取出n*l.w*l.h + i中i的值

    return batch*l.outputs + n*l.w*l.h*(l.coords+l.classes+1) + entry*l.w*l.h + loc;  //0*l.outputs + 3*7*7*(4+9413+1)+4*7*7+i
}
...
void get_region_detections(layer l, int w, int h, int netw, int neth, float thresh, int *map, float tree_thresh, int relative, detection *dets)
{
    ...
    for (i = 0; i < l.w*l.h; ++i){
        ...
        for(n = 0; n < l.n; ++n){
            ...
            int obj_index  = entry_index(l, 0, n*l.w*l.h + i, l.coords);  //参数是（l,0,n*w*h+i,4）返回是0*l.outputs + 3*7*7*(4+9413+1)+4*7*7+i
            int box_index  = entry_index(l, 0, n*l.w*l.h + i, 0);
            int mask_index = entry_index(l, 0, n*l.w*l.h + i, 4);
            float scale = l.background ? 1 : predictions[obj_index];
            dets[index].bbox = get_region_box(predictions, l.biases, n, box_index, col, row, l.w, l.h, l.w*l.h);  //(l.output,l.biases,3,box_index，col,row,l.w,l.h,l.w*l.h)
            dets[index].objectness = scale > thresh ? scale : 0;
            //printf("mask=%f,coords=%f",dets[index].mask,l.coords)
            if(dets[index].mask){
            ...
            //printf("class_index=%d",class_index);
            if(l.softmax_tree){
