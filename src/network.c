...
{
    network *net = parse_network_cfg(cfg);  //从配置文件yolov3.cfg中读取出来整个配置文件信息，并把网络加载net中
...
int num_detections(network *net, float thresh)  //检测大于阈值的类别有多少个
{
    int i;
    int s = 0;
    for(i = 0; i < net->n; ++i){
        layer l = net->layers[i];
        if(l.type == YOLO){
            s += yolo_num_detections(l, thresh);  //s加上大于阈值的个数
        }
...
detection *make_network_boxes(network *net, float thresh, int *num)  //参数（网络模型，阈值，box数量：初始的时候是0）
{
    layer l = net->layers[net-> n - 1];
    int i;
    int nboxes = num_detections(net, thresh);  //yolo预测的格子数量，等于w*h*n（anchor）
    if(num) *num = nboxes;  //num是一个地址，如果地址存在，则*num的值等于nboxes。
    detection *dets = calloc(nboxes, sizeof(detection));
    for(i = 0; i < nboxes; ++i){
        dets[i].prob = calloc(l.classes, sizeof(float));  //开辟一块存储概率的内存。
        if(l.coords > 4){
            dets[i].mask = calloc(l.coords-4, sizeof(float));
        }
    }
    return dets;  //开辟一块存储类所有box的预测结果的内存和存储类别概率值的内存
}
...
{
    detection *dets = make_network_boxes(net, thresh, num);  ////开辟一块存储类别概率值的内存和坐标的内存
    fill_network_boxes(net, w, h, thresh, hier, map, relative, dets);  //把类别信息，概率信息，预测出来的box信息放入dets中
    return dets;
}
